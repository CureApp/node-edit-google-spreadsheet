...
//public api
exports.getWorksheetInfos = function(opts, callback) {
  if (!callback)
    callback = opts.callback;
  if (!callback)
    throw "Missing callback";
  ...
  auth(opts, function(err, token) {
    ...
    spreadsheet.log('Logged into Google'.green);
    spreadsheet.setToken(token);
    spreadsheet.getSheetInfos(callback);
  });
};

Spreadsheet.prototype.getSheetInfos = function(callback) {
  this.request({
    url: this.protocol + '://spreadsheets.google.com/feeds/worksheets/' +
      this.spreadsheetId + '/private/full'
  }, function(err, result) {
    if (err) return callback(err);
    ...
    entries.forEach(function(entry) {
      var id = entry.id.substr(entry.id.lastIndexOf('/') + 1);
      infos.push({id:id, name:entry.title});
    });
